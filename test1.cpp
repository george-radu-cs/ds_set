/* test 1 - 1 set with few items: 15 */
...
int main() {
  ...
  /* create the data for the test and write to test1.in */
  long long int n = 15; /* no of elements */
  long long int lower_bound{-100}, upper_bound{100};
  ...
  /* write the data */
  std::ofstream create_fout("test1.in");
  ...
  for (long long int i = 0; i < number_testing; i++) {
    ...
    fout << "Time to find if " << v[index] << " is in the set: "
         << std::chrono::duration_cast<
                ...
                .count()
         << "s\n";
    if (rbt.isIn(v[index])) { /* should return true */
