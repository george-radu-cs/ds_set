/* test 3 - 1 set with few items: 10000 */
...
int main() {
  ...
  /* create the data for the test and write to test3.in */
  long long int n = 10000; /* no of elements */
  long long int lower_bound{-100000}, upper_bound{100000};
  ...
  /* write the data */
  std::ofstream create_fout("test3.in");
  create_fout << n + number_testing << '\n';
  for (long long int i = 0; i < n; i++) { /* add all the elements */
    create_fout << v[i] << " ";
  }
  for (long long int i = 0; i < number_testing;
       i++) { /* add some duplications */
  ...
  for (long long int i = 0; i < number_testing; i++) {
    ...
    fout << "Time to find if " << v[index] << " is in the set: "
         << std::chrono::duration_cast<
                std::chrono::duration<double, std::ratio<1>>>(
                std::chrono::high_resolution_clock::now() - timer)
                .count()
